...
class DHTPeer(Peer):
    ...
    def searchForKeywords(self, keywords):
        """ The DHT search operation (keyword-based)
    ...
    def publishData(self, name, data):
        """ The DHT high-level data publishing operation
        
        Call this to store data in the DHT DHT.
        ...
        wanted/needed, rather call the Kademlia base peer's
    ...
    def removeData(self, name):
        """ The DHT high-level data removal (delete) operation
        
        Call this to remove data from the DHT DHT.
        ...
        peer's C{iterativeDelete()} method directly.
        """
        h = hashlib.sha1()
        h.update(name)
        mainKey = h.digest()
    ...
    def iterativeDelete(self, key):
        """ The DHT delete operation
        
        Call this to remove data from the DHT.
        
        The DHT delete operation uses the basic Kademlia peer lookup
    ...
    def delete(self, key, **kwargs):
        ...
        this peer's data, and executes FIND_NODE for the key
        ...
        @return: A list of contact triples closest to the specified key. 
                 ...
                 peer is returning all of the contacts that it knows of.
        @rtype: list
        """
...
if __name__ == '__main__':
    ...
    from pydssim.network.protocol.dht.kademlia.datastore.sqllite_datastore import SQLLiteDataStore
    import sys, os
    if len(sys.argv) < 2:
    ...
    if os.path.isfile('C:\usuario\workspace\entangled\dbFile%s.db' % sys.argv[1]):
        os.remove('C:\usuario\workspace\entangled\dbFile%s.db' % sys.argv[1])
    dataStore = SQLLiteDataStore(dbFile = 'C:\usuario\workspace\entangled\dbFile%s.db' % sys.argv[1])
    peer = DHTPeer( udpPort=int(sys.argv[1]), dataStore=dataStore )
    #peer = DHTPeer( udpPort=int(sys.argv[1]) )
